...
namespace CastModelViewer.Util
{
    ...
    class SEModelImporter : ModelReader
    {
        ...
        ///Gets the extension of the current stream assuming its a filestream
        public static string GetStreamFileExtension(Stream stream)
        {
            // If the stream is a FileStream, return the FileStream.Name property.
        ...
        public override Model3DGroup Read(Stream s)
        {
            ...
            var ext = GetStreamFileExtension(s);
            if (ext == ".semodel")
                semodel = SEModel.Read(s);
            else if(ext == ".cast")
                semodel = SEModel.ReadCast(s);
